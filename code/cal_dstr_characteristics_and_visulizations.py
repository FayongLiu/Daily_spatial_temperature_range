...
# this file is for calculating the characteristics of DSTR based on ERA5-Land
...
def plotloc(axname1, axname2, axname3, locdata, lab, mkr, locclr, barnum):
    ...
    ssz = 2
    alp = 0.3
    if ffile in ['world', 'north_hemisphere', 'south_hemisphere']:
        gdf = worldgdf
        if ffile == 'world':
            min_lon, max_lon = -180, 180
            min_lat, max_lat = -90, 90
            gdf = gdf.cx[min_lon:max_lon, min_lat:max_lat]
        elif ffile == 'north_hemisphere':
            min_lon, max_lon = -180, 180
            min_lat, max_lat = 0, 90
            gdf = gdf.cx[min_lon:max_lon, min_lat:max_lat]
        elif ffile == 'south_hemisphere':
            min_lon, max_lon = -180, 180
        ...
        gdf = progdf
        gdf = gdf[gdf['pro'] == key]
        gdf.boundary.plot(ax=axname1, color='black', linewidth=1.0)
        wid = 0.5
        ssz = 3
...
# set basic info
# path
datapath = r'/data1/fyliu/a_temperature_range/process_data/DSTR_original_list/'
lspath = r'/data1/fyliu/a_temperature_range/process_data/DSTR_char/'
figpath = r'/data1/fyliu/a_temperature_range/result/fig_DSTR_char/'
...
china_coatline = r'/data1/fyliu/a_temperature_range/data/boundary/China_GS(2020)4619/China_coastline.shp'
...
for ffile in fall[-4:]:
    if ffile == 'Macau':
        ...
        # ffile for filename, key for figure
        ...
        if ffile not in ['world', 'north_hemisphere', 'south_hemisphere']:
        ...
        ax8.set_xlabel('Frequency', fontsize=8)
