...
# this file is for calculating DSTR in China based on ERA5-Land

# Initialize logging
logging.basicConfig(filename=r'/data1/fyliu/a_temperature_range/code/logging/data_processing_china.log', 
...
def cal_dstr(year):
    try:
        ...
        for tm in range(teststart, testend):
            ...
            # save clip data
            outpath2 = outpath + 'year_' + str(int(eyhmtr[ind, 0]))
            if not os.path.exists(outpath2):
                os.makedirs(outpath2)
            outname = 'china_t2m_' + str(int(eyhmtr[ind, 0])) + str(int(eyhmtr[ind, 1])).zfill(2) + str(int(eyhmtr[ind, 2])).zfill(2) + str(int(eyhmtr[ind, 3])).zfill(2) + '.nc'
            filepath = os.path.join(outpath2, outname)
...
def save_results(eyhmtr, eyhmaxt, eyhmint, year):
    ...
    outname1 = f'china_eachyear_maxTrange_{year}'
...
if __name__ == "__main__":
    ...
    # ERA5-Land hourly t2m data path
    path = r'/data_backup/share/ERA5_land/temperature/hourly/'
    # Extract the data to National scale, and save the results, when next time need to recalculate DSTR, it can be directly use this path
    outpath = r'/data_backup/fyliu/ERA5_Land/t2m/ERA5_Land_t2m_China/'
    # outpath of the DSTR
    outpath1 = r'/data1/fyliu/a_temperature_range/process_data/DSTR_original_list/china/'
    ...
    rasterize([(geom, 1) for geom in coun.geometry], out_shape=(num_lat, num_lon), transform=transform, fill=0, all_touched=True, out=grid)
